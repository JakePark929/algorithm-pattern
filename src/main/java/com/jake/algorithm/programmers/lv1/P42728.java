...
class P42728 {
    public LinkedList<Integer> solution(int[] array, int[][] commands) {
        ...
        for(int len = 0; len < start.size(); len++) {
            ...
            for(int i = start.get(len); i <= end.get(len); i++) {
                list.add(array[i - 1]);
            }
            Collections.sort(list);
            result.add(list.get(turn.get(len) - 1));
        }
    ...
    public static void main(String[] args) {
        P42728 problem = new P42728();
        System.out.println(Arrays.toString(problem.solution2(new int[]{1, 5, 2, 6, 3, 7, 4}, new int[][]{{2, 5, 3}, {4, 4, 1}, {1, 7, 3}})));
    }
}
