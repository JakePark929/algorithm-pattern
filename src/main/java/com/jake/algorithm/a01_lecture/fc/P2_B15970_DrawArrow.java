 ...
 * 점 b와 d의 화살표는 각각 d와 b로 연결된다. 또한 점 c와 e의 화살표는 각각 e와 c로 연결된다.
 * 따라서 모든 화살표들의 길이 합은 3 + 3 + 2 + 3 + 2 = 13이다.
 *
 * 점들의 위치와 색깔이 주어질 때, 모든 점에서 시작하는 화살표들의 길이 합을 출력하는 프로그램을 작성하시오.
 *
 * 입력
 * 표준 입력으로 다음 정보가 주어진다. 첫 번째 줄에는 점들의 개수를 나타내는 정수 N이 주어 진다.
 ...
 * 작성일 : 2026.02.26
 */
public class P2_B15970_DrawArrow {
    ...
    static void input() {
        ...
        for (int i = 1; i <= N; i++) {
            ...
            // color 인 색깔의 점이 coordinate 에 놓여있음
            a[color].add(coordinate);
        }
    }
    ...
    static void process() {
        ...
        for (int color = 1; color <= N; color++) {
            for (int i = 0; i < a[color].size(); i++) {
                int left = toLeft(color, i); // 왼쪽 점 까지 거리
                int right = toRight(color, i); // 오른 점 까지 거리
