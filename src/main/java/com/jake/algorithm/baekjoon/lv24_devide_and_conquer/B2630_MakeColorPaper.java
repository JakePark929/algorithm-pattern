...
class B2630_MakeColorPaper {
    ...
    private static void finder(int row, int col, int size) {
        ...
        finder(row, col, newSize);
        finder(row, col + newSize, newSize);
        finder(row + newSize, col, newSize);
        finder(row + newSize, col + newSize, newSize);
    }
    ...
    public static void cal(int p, int q, int num) {
        char color = arr[p][q];
        for (int i = p; i < p + num; i++) {
            for (int j = q; j < q + num; j++) {
                if (arr[i][j] != color) {
                    cal(p, q, num/2);
                    cal(p + num/2, q, num/2);
                    cal(p, q + num/2, num/2);
                    cal(p + num/2, q + num/2, num/2);
                    return;
                }
            }
        }
